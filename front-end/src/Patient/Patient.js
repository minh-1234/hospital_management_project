...
const Patient = ({ role }) => {
  ...
  useEffect(() => {
    const getAllPatient = async () => {
      try {
        const response = await axios.get(`${env.URL_BACKEND}/v1/patients`, { withCredentials: true });
        console.log(response.data);
        const dataWithStatusAndAge = await Promise.all(response.data.map(async item => {
          const age = getPatientAge(item.dateOfBirth);
          const treatProcessPromise = await axios.get(`${env.URL_BACKEND}/v1/patients/${item.id}/treatProcess`, { withCredentials: true });
