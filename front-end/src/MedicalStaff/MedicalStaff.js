...
const MedicalStaff = ({ role }) => {
  ...
  useEffect(() => {
    const getAllMedicalStaff = async () => {
      try {
        const response = await axios.get(`${env.URL_BACKEND}/v1/specialists?position=` +
        ...
        const dataWithStatusAndAge = await Promise.all(await response.data.map(async item => {
          ...
          const response_schedule = await axios.get(`${env.URL_BACKEND}/v1/specialists/${item.id}/schedules`, { withCredentials: true });
