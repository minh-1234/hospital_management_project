...
const DetailMedicalStaff = () => {
  ...
  useEffect(() => {
    const getMedicalStaff = async () => {
      try {
        let response = await axios.get(`${env.URL_BACKEND}/v1/specialists/` + id, { withCredentials: true });
        const schedule = await axios.get(`${env.URL_BACKEND}/v1/specialists/${id}/schedules`, { withCredentials: true })
