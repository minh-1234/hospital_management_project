...
const ScheduleProcess = ({ medicalStaff }) => {
  ...
  const handleDeleteSchedule = () => {
    axios
      .delete(`${env.URL_BACKEND}/v1/specialists/${id}/schedules/${scheduleID}`)
  ...
  const handleSubmit = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      axios
        .put(`${env.URL_BACKEND}/v1/specialists/${id}/schedules/${scheduleID}`, newSchedule)
        .then(() => {
    ...
    <div>
      {medicalStaff.schedule.map((schedule, index) => (
        <div className="treat-circle-block">
          <div
            className={`circle circle-${moment(
