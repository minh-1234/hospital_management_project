...
const DetailPatient = () => {
  ...
  useEffect(() => {
    const getPatient = async () => {
      try {
        let response = await axios.get(`${env.URL_BACKEND}/v1/patients/` + id, { withCredentials: true });
        const treatProcessPromise = await axios.get(`${env.URL_BACKEND}/v1/patients/${id}/treatProcess`, { withCredentials: true });
