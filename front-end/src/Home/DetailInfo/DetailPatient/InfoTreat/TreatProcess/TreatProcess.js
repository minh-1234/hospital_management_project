...
const TreatProcess = ({ patient }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const addTreatProcess = async () => {
        axios
          .put(
            `${env.URL_BACKEND}/localhost:8080/v1/patients/${id}/treatProcess/${curTreatProcess.id}`,
            newTreatProcess
          )
          .then(() => {
    ...
    const getMedStaff = async () => {
      try {
        const response = await axios.get(
          `${env.URL_BACKEND}/v1/specialists?` + queryStr
        )
        let dataWithSchedule = await Promise.all(await response.data.map(async item => {
          const response_schedule = await axios.get(`${env.URL_BACKEND}/v1/specialists/${item.id}/schedules`);
  ...
  const handleDeleteTreatProcess = (idTP) => {
    axios
      .delete(
        `${env.URL_BACKEND}/v1/patients/${id}/treatProcess/${idTP}`
      )
    ...
    <div>
      {patient.treatProcess.map((treatment, index) => (
        <div key={index} className="treat-circle-block">
          <div
            className={`circle circle-${moment(treatment.dateEnd, "DD-MM-YYYY") < moment()
                ? index === 0
                  ? "complete"
                  : "complete-task"
                : "on-going"
          ...
          <div className="treat-block">
            <div
              className={`icon-in-treatform ${index === isUpdate ? "close" : null
                }`}
            >
              <FontAwesomeIcon
            ...
            {!(index === isUpdate) ? (
              <div>
                ...
                <div className="specialist">
                  ...
                  <span>
                    <a
                      href={`/MedicalStaff/${treatment.position === "Bác sĩ" ? "specialist" : treatment.position === "Y tá" ? "nurse" : "supportStaff"
